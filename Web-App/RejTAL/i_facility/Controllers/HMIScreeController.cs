    ...
    public class HMIScreeController : Controller
    {
        ...
        public ActionResult Index(int id = 0, string selectedValues = "")
        {
            ...
            if (HMI.Count == 0)
            {
                ...
                if (HMIOpName != null)
                {
                    //Session["OpName"] = HMIOpName.OperatorDet;  //commented on 2020-12-05 for operator login
                }
                ...
                foreach (var plan in tbldaily_plan)
                {
                    ...
                    if (hmiid != ExceptionHMIID)
                    {
                        ...
                        if (hmiidData != null)
                        {
                            ...
                            //2017-01-21
                            //using (i_facility.Models.i_facility_talEntities dbHMI = new i_facility.Models.i_facility_talEntities())
                            //{
                            ...
                            int RejectedNew = Convert.ToInt32(hmiidData.RejectionQuantity);
                            int ProcessedNew = Convert.ToInt32(hmiidData.ProcessQty);
                            newProcessedQty = DeliveredNew + ProcessedNew + RejectedNew;
                ...
                case "cjtextbox22":
                    {
                        if (value != "")
                        {
                            ...
                            thisrow[0].RejectionQuantity = Convert.ToInt32(value);
                        ...
                        {
                            //  thisrow[0].Delivered_Qty = null;
                            thisrow[0].RejectionQuantity = null;

                        }
                        obj.UpdateLiveHMIScreenDetailsD1(thisrow[0].HMIID, Convert.ToString(thisrow[0].RejectionQuantity));
                        break;
                    }
        ...
        //    {
        //        //using (i_facility_talEntities dbsimilar = new i_facility_talEntities())
        //        //{
        ...
        public ActionResult OverAllPartial(string HMIIDs)
        {
                ...
                if (HMIData != null)
                {
                    ...
                    else if (HMIData.Date != null)
                    {
                        ...
                        if (int.TryParse(Convert.ToString(HMIData.Delivered_Qty), out deliveredQty))
                        {
                            ...
                            int rejectionQty = 0;
                            int.TryParse(Convert.ToString(HMIData.RejectionQuantity), out rejectionQty);

                            if ((deliveredQty + processed + rejectionQty) > Convert.ToInt32(HMIData.Target_Qty))
                            {
                                Session["VError"] = "DeliveredQty and rejectionQty Must be less than Target for " + HMIData.Work_Order_No;
                                return RedirectToAction("Index");
                            }

                            #region 2017-02-07
                            else if ((deliveredQty + processed + rejectionQty) == Convert.ToInt32(HMIData.Target_Qty))
                            {
                                //Check if highest of opno's for sibling wo's is eligible for JF, if Qty satisfies.
                                //1) if already lower op is jobfinished
                                //2) or  its in current HMIIDArray and valid for jf
            ...
            foreach (string hmiid in HMIIDArray)
            {
                ...
                if (hmiidi != ExceptionHMIID)
                {
                    ...
                    int DelivQty = 0, TargQty = 0, ProcessedQty = 0, rejectionQty = 0;

                    int.TryParse(HMIData.Target_Qty.ToString(), out TargQty);
                    int.TryParse(HMIData.Delivered_Qty.ToString(), out DelivQty);
                    int.TryParse(HMIData.ProcessQty.ToString(), out ProcessedQty);

                    int.TryParse(HMIData.RejectionQuantity.ToString(), out rejectionQty);

                    if (TargQty >= (DelivQty + ProcessedQty + rejectionQty))
                    {
                        string WoNo = HMIData.Work_Order_No;
                        string partNo = HMIData.PartNo;
                        int OpNo = Convert.ToInt32(HMIData.OperationNo);
                        foreach (string hmiid1 in HMIIDArray)
                        {
                            ...
                            if (hmiidi != ExceptionHMIID)
                            {
                                ...
                                if (HMIDataInner != null)
                                {
                                    ...
                                    if (opNoInner < OpNo)
                                    {
                                        int DelivQty1 = 0, TargQty1 = 0, ProcessedQty1 = 0, rejectionQty1 = 0;
                                        int.TryParse(HMIDataInner.Target_Qty.ToString(), out TargQty1);
                                        int.TryParse(HMIDataInner.Delivered_Qty.ToString(), out DelivQty1);
                                        int.TryParse(HMIDataInner.ProcessQty.ToString(), out ProcessedQty1);
                                        int.TryParse(HMIDataInner.ProcessQty.ToString(), out rejectionQty1);
                                        if (TargQty1 < (DelivQty + ProcessedQty + rejectionQty1))
                                        {
                                            Session["VError"] = "Click JobFinish, All WorkOrders have (Delivered + Processed+ rejection) = Target.";
                                            IsSequenceOK = false;
                                            break;
                                        }
                                    }
            ...
            foreach (string hmiid in HMIIDArray)
            {
                ...
                if (hmiidi != ExceptionHMIID)
                {
                    ...
                    int.TryParse(HMIData.RejectionQuantity.ToString(), out rejqty);
                    ...
                    {
                        int InnerProcessed = row.ProcessQty;
                        int FinalProcessed = DelivQty + InnerProcessed + rejqty;
                        if (FinalProcessed < row.Target_Qty)
                        {
                            if (row.isWorkInProgress == 2)
                            {
            ...
            foreach (string hmiid in HMIIDArray)
            {
                ...
                if (hmiidi != ExceptionHMIID)
                {
                    ...
                    int.TryParse(HMIData.ProcessQty.ToString(), out ProcessedQty);
                    int.TryParse(HMIData.RejectionQuantity.ToString(), out rejqty);
                    if (TargQty == (DelivQty + ProcessedQty + rejqty))
                    {
                        string wono = HMIData.Work_Order_No;
                        string partno = HMIData.PartNo;
                        string opno = HMIData.OperationNo;
                    ...
                    else if (TargQty > (DelivQty + ProcessedQty + rejqty))
                    {
                        HMIData.Status = 1;
                        HMIData.isWorkInProgress = 0;
                    }
                    ...
                    {
                        Session["VError"] = "Delivered + Processed + rejection qty Cannot be Greater than Target for :" + HMIData.Work_Order_No;
                        return RedirectToAction("Index");
                    }
                    HMIData.Time = DateTime.Now;
                    int HMIID = HMIData.HMIID;
        ...
        public ActionResult OverAllFinish(string HMIIDs)
        {
            ...
            foreach (string hmiid in HMIIDArray)
            {
                ...
                {
                    int deliveredQty = 0, processQty = 0, rejectionQty = 0;
                    int.TryParse(Convert.ToString(HMIData.ProcessQty), out processQty);
                    int.TryParse(Convert.ToString(HMIData.RejectionQuantity), out rejectionQty);

                    if (int.TryParse(Convert.ToString(HMIData.Delivered_Qty), out deliveredQty))
                    {
                        if ((deliveredQty + processQty + rejectionQty) == Convert.ToInt32(HMIData.Target_Qty))
                        {
                        }
                        else
                        {
                            Session["VError"] = "DeliveredQty and rejectionQty Must be equal to Target Qty";
                            return RedirectToAction("Index");
                        }
                    }
                    else
        ...
        public string OperatorDetails(string Operatorid)
        {
            ...
            using (Models.i_facility_talEntities db = new Models.i_facility_talEntities())
            {
        ...
        public ActionResult DDLList(string data1, string selectedValue = "")
        {
            ...
            if (WOCount == 1)
            {
                ...
                if (isValid)
                {
                    ...
                    int PrvProcessQty = 0, PrvDeliveredQty = 0, PrvRejectionQty = 0, TotalProcessQty = 0, ishold = 0;
                    ...
                    if (isHMIFirst == 1)
                    {
                        ...
                        string rejString = Convert.ToString(hmiData[0].RejectionQuantity);
                        int rejInt = 0;
                        int.TryParse(rejString, out rejInt);

                        PrvProcessQty += procInt;
                        PrvDeliveredQty += delivInt;
                        PrvRejectionQty += rejInt;
                    ...
                    #endregion
                    TotalProcessQty = PrvProcessQty + PrvDeliveredQty + PrvRejectionQty;
        ...
        public JsonResult checkPassword(string OperatorID, string password)
        {
            ...
            using (Models.i_facility_talEntities db = new Models.i_facility_talEntities())
            {
                Models.tbluser OPDet = db.tblusers.Where(m => m.IsDeleted == 0 && m.UserName == OperatorID && m.Password == password).FirstOrDefault();
                if (OPDet != null)
                {
                    ValidOp = "true";

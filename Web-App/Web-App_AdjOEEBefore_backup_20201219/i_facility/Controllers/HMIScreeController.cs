    ...
    public class HMIScreeController : Controller
    {
                ...
                foreach (var plan in tbldaily_plan)
                {
                    ...
                    if (hmiid != ExceptionHMIID)
                    {
                        ...
                        if (hmiidData != null)
                        {
                            ...
                            bool IsInHMI = true;
                            //int OperationNoInt = Convert.ToInt32(opNo);
                            //string WIPQuery1 = @"SELECT * from tblddl where WorkOrder = '" + woNo + "' and PartName = '" + partNo + "' and OperationNo != '" + opNo + "'  and IsCompleted = 0 order by WorkOrder,PartName,OperationNo  ";
                            ...
                            foreach (var row in WIPDDL)
                            {
                                ...
                                if (InnerOpNo < OperationNoInt)
                                {
                                    ...
                                    {
                                        //Session["VError"] = " Select & Start WONo: " + row.WorkOrder + " and PartNo: " + row.PartName + " and OperationNo: " + InnerOpNo;
                                        //return RedirectToAction("Index");
                                        IsInHMI = false;
                                    }
                                    else
                                    {
                                        foreach (var rowHMI in WIP)
                                        {
                                            ...
                                            {
                                                Session["VError"] = " Start WONo: " + row.WorkOrder + " and PartNo: " + row.PartName + " and OperationNo: " + InnerOpNo;
                                    ...
                                    if (!IsInHMI)
                                    {
                                        ...
                                        {
                                            Session["VError"] = " Select  WONo: " + row.WorkOrder + " and PartNo: " + row.PartName + " and OperationNo: " + InnerOpNo;
                                            return RedirectToAction("Index");
                                            //IsInHMI = false;
                                            //break;
                                        }
                            ...
                            if (TargetQtyNew == newProcessedQty)
                            {
                                ...
                                //var DDLList = db.tblddls.Where(m => m.WorkOrder == hmiidData.Work_Order_No && m.PartName == hmiidData.PartNo && m.OperationNo == hmiidData.OperationNo && m.IsCompleted == 0).ToList();
                    ...
                    else if (HMIData.Date != null)
                    {
                        ...
                        if (int.TryParse(Convert.ToString(HMIData.Delivered_Qty), out deliveredQty))
                        {
                            ...
                            else if ((deliveredQty + processed) == Convert.ToInt32(HMIData.Target_Qty))
                            {
                                ...
                                //string WIPQuery1 = @"SELECT * from tblddl where WorkOrder = '" + wono + "' and PartName = '" + partno + "'  and OperationNo != '" + opno + "' and IsCompleted = 0 order by OperationNo ";
            ...
            foreach (var hmiid in HMIIDArray)
            {
                ...
                if (hmiidi != ExceptionHMIID)
                {
                    ...
                    if (TargQty == (DelivQty + ProcessedQty))
                    {
                        ...
                        //2017-01-19
                        //var ddldata1 = db.tblddls.Where(m => m.WorkOrder == wono && m.PartName == partno).FirstOrDefault();
                        //if (ddldata1 != null)
                        //{
                        //    int opLocal = 0;
                        ...
                        //var ddldata = db.tblddls.Where(m => m.WorkOrder == wono && m.PartName == partno && m.OperationNo == opno).FirstOrDefault();
                        if (ddldata != null)
                        {
        ...
        public ActionResult OverAllFinish(string HMIIDs)
        {
            ...
            foreach (var rowOuter in WIPOuter)
            {
                    ...
                    else
                    {
                        if (OperationNoInt > InnerOpNo)
                        {
                            if (row.IsCompleted != 1) //=> lower OpNo is not Finished.
                            {
                                ...
                                if (!retStatus)
                                {
                                    Session["VError"] = " Finish WONo: " + row.WorkOrder + " and PartNo: " + row.PartName + " and OperationNo: " + outop;
                                    return RedirectToAction("Index");
                                }
                                else { }
                            }
                ...
                //        string WIPQuery1 = @"SELECT * from tblddl where WorkOrder = '" + multiWO + "' and PartName = '" + multiPart + "'  and OperationNo != '" + multiOP + "' order by OperationNo ";
                //        var WIP1 = dbHMI.tblddls.SqlQuery(WIPQuery1).ToList();
                //        foreach (var row in WIP1)
                //        {
                //            int InnerOpNo = Convert.ToInt32(row.OperationNo);
                ...
                //                    if (row.IsCompleted != 1) //=> lower OpNo is not Finished.
                //                    {
                //                        Session["VError"] = " Finish WONo: " + row.WorkOrder + " and PartNo: " + row.PartName + " and OperationNo: " + InnerOpNo;
                //                        return RedirectToAction("Index");
                //                    }
                //                }
                //            }
        ...
        public ActionResult Edit(int id = 0, int reworkorderhidden = 0, int cjtextbox9 = 0, int cjtextbox8 = 0)
        {
            ...
            if (IsWOMultiWO == 0)
            {
                ...
                //string WIPQuery6 = @"SELECT * from tblddl where WorkOrder = '" + woNo + "' and PartName = '" + partNo + "' and OperationNo != '" + opNo + "'  and IsCompleted = 0 order by WorkOrder,PartName,OperationNo ";
                ...
                foreach (var row in WIPDDL1)
                {
                    ...
                    if (InnerOpNo < OperationNoInt)
                    {
                        ...
                        {
                            Session["Error"] = " Finish WONo: " + row.WorkOrder + " and PartNo: " + row.PartName + " and OperationNo: " + InnerOpNo;
                            return RedirectToAction("Index");
                        }
                        else
                        {
                ...
                //var DDLData = db.tblddls.Where(m => m.PartName == partNo && m.OperationNo == opNo && m.WorkOrder == woNo).FirstOrDefault();
                var DDLData = obj.GetddlDetails3(woNo, partNo, opNo);
                if (DDLData != null)
                {
                    //DDLData.IsCompleted = 1;
        ...
        public void IntoFile(string Msg)
        {
            try
            {
                ...
                using (StreamWriter writer = new StreamWriter(appPath, true)) //true => Append Text
                {
                    ...
                    writer.WriteLine(System.DateTime.Now + ":   cultureinfo  : "+newCulture.DateTimeFormat.ShortDatePattern +" : " + Msg + "\r \n");
                }
            }
            catch (Exception e7)
            {
...

        {
            ...
            if (DDLID != 0)
            {
                ...
                //bool IsInHMI = false;
                //var Siblingddldata = db.tblddls.Where(m => m.IsCompleted == 0 && m.WorkOrder == WONo && m.PartName == Part && m.OperationNo != Operation).OrderBy(m => new { m.WorkOrder, m.PartName, m.OperationNo }).ToList();
                ...
                //            {
                //                Session["VError"] = " Start WONo: " + row.WorkOrder + " and PartNo: " + row.PartName + " and OperationNo: " + localOPNoString;
                //                //return RedirectToAction("Index");
                //                IsInHMI = false;
                //                //break;
                //            }
                ...
                //            if (WIPMWO.Count == 0)
                //            {
                //                Session["VError"] = " Select  WONo: " + row.WorkOrder + " and PartNo: " + row.PartName + " and OperationNo: " + localOPNoString;
                //                //return RedirectToAction("Index");
                //                //IsInHMI = false;
                //                //break;
                //                return View();
                ...
                //hmidata.PartNo = ddldata.PartName;
           ...
           // if (int.TryParse(workidstring, out workcntrcode) && int.TryParse(cellidstring, out cellid) && int.TryParse(shopidstring, out shopid))
            ...
            foreach (var row in WIP)
            {
                ...
                //var ddldata = db.tblddls.Where(m => m.WorkOrder == wono && m.PartName == partno && m.OperationNo == opno && m.IsCompleted == 0).FirstOrDefault();
                if (ddldata != null)
                {
            ...
            if (WOCount == 1)
            {
                ...
                if (DDLID != 0)
                {
                    ...
                    if (isValid)
                    {
                        ...
                        // var Siblingddldata = db.tblddls.Where(m => m.IsCompleted == 0 && m.WorkOrder == WONoInner && m.PartName == PartInner && m.OperationNo != OperationInner && m.IsCompleted == 0).OrderBy(m => new { m.WorkOrder, m.PartName, m.OperationNo }).ToList();
                        foreach (var row in Siblingddldata)
                        {
                            int localOPNo = Convert.ToInt32(row.OperationNo);
                            string localOPNoString = Convert.ToString(row.OperationNo);
                            if (localOPNo < Convert.ToInt32(OperationInner))
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        IssueMsg = " Start WONo: " + row.WorkOrder + " and PartNo: " + row.PartName + " and OperationNo: " + localOPNoString;
                                        //return RedirectToAction("Index");
                                        IsInHMI = false;
                                        isValid = false;
                                        break;
                                    ...
                                    else
                                    {
                                        ...
                                        if (LeastOperation != null)
                                        {
                                            ...
                                            if (!retstatus)
                                            {
                                                IssueMsg = " Select  WONo: " + LeastOperation.WorkOrder + " and PartNo: " + LeastOperation.PartName + " and OperationNo: " + outOperation;
                                ...
                                //    if (WIPMWO.Count == 0)
                                //    {
                                //        IssueMsg = " Select  WONo: " + row.WorkOrder + " and PartNo: " + row.PartName + " and OperationNo: " + localOPNoString;
                                //        //return RedirectToAction("Index");
                                //        isValid = false;
                                //        IsInHMI = false;
                                //        break;
            ...
            else if (WOCount > 1)
            {
                ...
                foreach (var DDLRow in DDLData)
                {
                                ...
                                if (ddlList != null)
                                {
                                    ...
                                    if (HMI != null)
                                    {
                                        if (HMI.Count == 0)
                                        {
                                            ...
                                            //var IssueMsg = " Select  WONo: " + DDL1.WorkOrder + " and PartNo: " + DDL1.PartName + " and              OperationNo: " + DDL1.OperationNo;
                                            ViewBag.toaster_error = " Select  WONo: " + DDL1.WorkOrder + " and PartNo: " + DDL1.PartName + " and OperationNo: " + DDL1.OperationNo;
                                            //Session["Error"] = IssueMsg;
                                            //Session.Timeout = 1;
                                            isValid = false;
                                        }
                            ...
                            if (ddlList != null)
                            {
                                ...
                                if (HMI != null)
                                {
                                    if (HMI.Count == 0)
                                    {
                                        ...
                                        obj1.PartNo = DDL1.PartName;
                                        obj1.Errmsg = "Select Work Order:" + DDL1.WorkOrder + " Operation No:" + DDL1.OperationNo + " Part No:" + DDL1.PartName + " First";
                                        HmiList.Add(obj1);
                                        isValid = false;
                                    }
                                }
                    ...
                    if (isValid)
                    {
                        ...
                        obj.InsertLiveHMIDetailsddl(DateTime.Now, ddlList.OperationNo, ddlList.PartName, ddlList.Project, CorrectedDate, Shift, ddlList.Type, Convert.ToInt32(Session["UserID"]), OpName, Convert.ToInt32(ddlList.TargetQty), ddlList.WorkOrder, hmidata.isWorkOrder, TotalProcessQty, 0, ddlList.WorkCenter, machineId);
                    }
                }
        ...
        public bool CheckAlltheWO(string woNo, string opNo, string partNo, out string OperationNum)
        {
            ...
            {
                Hmi hmiobj = new Hmi();
                var livehmidet = obj.validatelivehmiscreensDet(ddlrow.WorkOrder, ddlrow.PartName, ddlrow.OperationNo);
                if (livehmidet != null)
                {
                ...
                {
                    var histhmidet = obj.validateHisthmiscreensDet(ddlrow.WorkOrder, ddlrow.PartName, ddlrow.OperationNo);
                    if (histhmidet != null)
                    {
        ...
        public ActionResult PartialFinishedList(int DDLID = 0, string MacInvNo = null, int ToHMI = 0)
        {
            ...
            if (DDLID != 0)
            {
                ...
                obj.UpdateLiveHMIScreenDetails6(ddldata.OperationNo, ddldata.PartName, ddldata.Project, Target_Qty, ddldata.WorkOrder, TotalProcessQty, ddldata.WorkCenter, Hmiid);
                //hmidata.OperationNo = ddldata.OperationNo;
                //hmidata.PartNo = ddldata.PartName;
                //hmidata.Project = ddldata.Project;
        ...
        public ActionResult PartialFinishedList(List<int> data)
        {
            ...
            if (WOCount == 1)
            {
                ...
                //db.SaveChanges();
                int Target_Qty = Convert.ToInt32(ddldata.TargetQty);
                obj.UpdateLiveHMIScreenDetails3(Hmiid, ddldata.OperationNo, ddldata.PartName, ddldata.Project, Target_Qty, ddldata.WorkOrder);
                //hmidata.OperationNo = ddldata.OperationNo;
                //hmidata.PartNo = ddldata.PartName;
                //hmidata.Project = ddldata.Project;
                    ...
                    if (ddlData != null)
                    {
                        ...
                        if (ddlWorkCenter != null)
                        {
                            ...
                            if (ddlWorkCenterMacDetails != null)
                            {
                                ...
                                {
                                    obj.UpdateLiveHMIScreenDetails(ddlData.WorkCenter, hmiData.HMIID, isHold, tragetqty, ddlData.Type, MachineID, OpNo, ddlData.PartName, ddlData.Project, WONo);
        ...
        public ActionResult IndividualSubmit(int HMIID = 0)
        {
            ...
            if (hmiidDataDup != null)
            {
                ...
                //var DDLCompletedData = db.tblddls.Where(m => m.WorkOrder == woNo && m.PartName == partNo && m.OperationNo == opNo && m.IsCompleted == 1).FirstOrDefault();
                ...
                if (isValid)
                {
                    //var Siblingddldata = db.tblddls.Where(m => m.IsCompleted == 0 && m.WorkOrder == woNo && m.PartName == partNo && m.OperationNo != opNo && m.IsCompleted == 0).OrderBy(m => new { m.WorkOrder, m.PartName, m.OperationNo }).ToList();
            ...
            if (hmiidData.IsHold == 0)
            {
                if (hmiidData.Date == null)
                {
                    if (hmiidData.OperatorDet != null && hmiidData.Shift != null && hmiidData.Project != null && hmiidData.Prod_FAI != null && hmiidData.PartNo != null && hmiidData.Work_Order_No != null && hmiidData.OperationNo != null && hmiidData.Target_Qty.HasValue)
                    {
                        ...
                        bool IsInHMI = true;
                        //string WIPQuery2 = @"SELECT * from tblddl where WorkOrder = '" + woNo + "' and PartName = '" + partNo + "' and OperationNo != '" + opNo + "'  and IsCompleted = 0 order by WorkOrder,PartName,OperationNo ";
                ...
                if (hmiidData.IsHold != 1)
                {
                    if (hmiidData.Date != null)
                    {
                        ...
                        if ((cjtextbox8 + ProcessQty) == hmiidData.Target_Qty)
                        {
                            ...
                            #region 2017-07-01
                            //New Logic to Overcome WorkOrder Sequence Scenario 2017-02-03
                            //string WIPQuery6 = @"SELECT * from tblddl where WorkOrder = '" + wono + "' and PartName = '" + partno + "' and OperationNo != '" + opno + "' and IsCompleted = 0 order by OperationNo ";
                            //var WIPDDL6 = db.tblddls.SqlQuery(WIPQuery6).ToList();
